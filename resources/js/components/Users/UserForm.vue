<template>
    <div>
        <p class="text-center py-2 bg-navy text-white">編輯</p>
        <div class="p-4">
            <vue-form :url="`/admin/users/${user_id}`" :form-model="form" @submission-okay="userUpdated">
                <div slot-scope="{formData, formErrors, waiting}">
                    ...
                    <div class="my-4 flex justify-end">
                        <button type="button" class="btn btn-white" @click="$emit('cancel')">取消</button>
                        <button class="btn btn-orange ml-4" type="submit">更新</button>
                    </div>
                </div>
            </vue-form>
